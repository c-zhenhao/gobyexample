...
func main() {
	...
	// here's a non-blocking receive. if a value is avialable on messages then select will take the <-messages case with that value. if not it will immediately take the default case
	select {
	case msg := <-messages:

// sometimes we'll want to sort a collection by something other than its natural order. For example, suppose we wanted to sort strings by length instead of alphabetically. Here's an example of custom sorts in Go
package main
...
// running the program shows a list of strings sorted by length, as desired
